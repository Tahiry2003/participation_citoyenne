...
export default function Doleance({ doleance, user, onDeleted, onSupportUpdate, userRole }) {
  ...
  const handleValidate = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        setIsValidated(true);
        showNotification("success", "Doléance validée avec succès !");
      } else {
  ...
  const handleReject = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        setIsRejeter(true);
        showNotification("success", "Doléance rejetée avec succès !");
      } else {
        showNotification("error", data.error || "Erreur lors de la rejet");
      }
    } catch (err) {
      console.error(err);
      showNotification("error", "Erreur de connexion au serveur");
  ...
  const handleStartTraitement = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification("success", "Doléance en cours de traitement !");
      } else {
  ...
  const handleResolu = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        setIsResolue(true);
        setIsTraiter(false);
        showNotification("success", "Doléance resolue");
      } else {
        showNotification("error", data.error || "Erreur lors de la resolution");
      }
    } catch (err) {
      console.error(err);
      showNotification("error", "Erreur de connexion au serveur");
  ...
  const renderSupportButton = useMemo(() => {
    ...
    // Vérifier si l'utilisateur connecté est un citoyen
    // Note: vous devrez peut-être adapter cette logique selon comment vous stockez le rôle
    ...
    // CORRECTION: hasSupported utilise maintenant userSupport qui est mis à jour immédiatement
    return (
      <button
